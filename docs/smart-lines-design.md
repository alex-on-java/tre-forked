...
### 0. Respect Gitignore (Preserve Default Behavior)

**Critical**: `tre` by default respects `.gitignore` and `.git/info/exclude` files. This behavior **must be preserved** when using `--lines`.

```bash
# Default: respects gitignore (node_modules, build artifacts, etc. are hidden)
tre --lines 50

# With -a flag: shows ALL files including ignored ones
tre -a --lines 50
```

This means:
- `node_modules/`, `build/`, `.cache/` etc. are typically **not shown** by default
- The line budget applies to **tracked/non-ignored files only**
- When `-a` is used, ignored directories can be massive — the algorithm must handle this gracefully

**Why this matters for AI agents**: The default gitignore-respecting behavior already filters out noise (dependencies, build outputs). The `--lines` feature builds on this clean foundation.
...
└── package.json
```
Note: `node_modules/` is not shown because it's in `.gitignore`.

**With -a flag (shows everything including ignored):**
```bash
$ tre -a --lines 50
```
```
...
When `-a` is used, the algorithm must handle massive ignored directories gracefully.
